...
export default function Home() {
  ...
  const advice = useMemo(() => buildAdvice(mode, daily, intelligence, compositePrice), [mode, daily, intelligence, compositePrice]);
  ...
  const klineReason = useMemo(() => buildKlineReason(daily, intelligence, compositePrice, forecast?.direction), [daily, intelligence, compositePrice, forecast?.direction]);
  ...
  return <main className="app-shell" id="overview">
    ...
    <div className="dashboard">
      <section className="market-hero">
        <div className="hero-topline"><div><div className="pair-line"><strong>BTC 综合价</strong><span>{currentMarkets.length}/{exchanges.length} LIVE</span></div><div className="price-line"><h1>{formatPrice(compositePrice)}</h1><span className={(compositeChange ?? 0) < 0 ? 'negative' : 'positive'}>{signed(compositeChange ?? undefined)}</span></div><p className="source-line">最后更新 {ageLabel(latestUpdate || null, now)} · 5 EXCHANGES</p></div><div className="market-stats"><div><span>24h 高</span><b>{formatPrice(high24h)}</b></div><div><span>24h 低</span><b>{formatPrice(low24h)}</b></div><div><span>五所成交</span><b>{formatVolume(volume24h)}</b></div></div></div>
        ...
        <div className="chart-toolbar"><div className="timeframes">{historyOptions.map((item) => <button key={item} className={range === item ? 'active' : ''} onClick={() => setRange(item)}>{item}</button>)}</div><div className="chart-legend"><span className="dot orange" />真实收盘 <span className="dot purple" />虚拟K线 <span className="dot green" />成交量</div></div>
        ...
        {forecast && <div className="virtual-timeline" aria-label="虚拟K线精确时间">{forecast.virtualCandles.map((candle, index) => <button type="button" className={selectedCandleIndex === index ? 'active' : ''} key={candle.time} onClick={() => setSelectedCandleIndex(index)} aria-expanded={selectedCandleIndex === index}><time>{formatCandleTime(candle.time, marketInterval)}</time><b className={candle.close >= candle.open ? 'positive' : 'negative'}>{formatPrice(candle.close)}</b><span>第{index + 1}根 · {candle.close >= candle.open ? '预计阳线' : '预计阴线'} · 点击看原因</span></button>)}</div>}
        {selectedCandle && <section className="candle-explanation" aria-live="polite">
        ...
        <div className="daily-source">{daily?.source ?? '市场数据'} · 原始K线 UTC · 预测时间 JST · 紫色区域为情景推演</div>
      </section>

      <aside className="signal-card panel"><div className="panel-heading"><div><span className="eyebrow">AI {intervalName(marketInterval)}K判断 · 实时计算</span><h2>{advice.label}</h2></div><div className="mode-switch"><button className={mode === 'short' ? 'active' : ''} onClick={() => setMode('short')}>短线</button><button className={mode === 'long' ? 'active' : ''} onClick={() => setMode('long')}>长线</button></div></div><div className="score-block"><div className="score-ring" style={{ '--score': `${advice.score * 3.6}deg` } as React.CSSProperties}><div><b>{advice.score}</b><span>/ 100</span></div></div><div><span className="score-tag">{advice.bias}</span><p>{advice.detail}</p></div></div><div className="ai-reason"><span>为什么{intervalName(marketInterval)}K这样走</span><p>{klineReason}</p></div><div className="signal-section-label">关键数据</div><div className="signal-points">{advice.points.map((point) => <div key={point.text}><i className={point.positive ? '' : 'warn'}>{point.positive ? '✓' : '!'}</i><span>{point.text}</span></div>)}</div><div className="action-zone"><div><span>当前动作</span><b>{advice.allocation}</b></div><div><span>{advice.score >= 58 ? '防守位置' : '重新转强'}</span><b>{formatPrice(advice.invalidation)}</b></div></div><button className="primary-button" onClick={() => setAlerts(true)}>开启价格提醒</button><p className="panel-note">按市场数据计算，不自动交易</p></aside>
