    ...
    <div className="dashboard">
      <section className="market-hero">
        ...
        <div className="market-workspace">
          ...
          {forecast && <aside className="forecast-inspector" aria-label="全部虚拟K线分析"><div className="inspector-heading"><div><span className="eyebrow">逐根K线分析</span><h3>全部在这里查看</h3></div><b>{forecast.virtualCandles.length} 根 · {forecast.label}</b></div><div className="candle-accordion-list">{forecast.virtualCandles.map((candle, index) => { const active = selectedCandleIndex === index; return <article className={active ? 'active' : ''} key={candle.time}><button type="button" className="candle-accordion-trigger" onClick={() => setSelectedCandleIndex(index)} aria-expanded={active}><div><time>{formatCandleTime(candle.time, marketInterval)}</time><strong className={candle.close >= candle.open ? 'positive' : 'negative'}>{formatPrice(candle.close)}</strong></div><span className={candle.close >= candle.open ? 'up' : 'down'}>{candle.close >= candle.open ? '走高' : '走低'} · {candle.confidence}%</span><i>{active ? '−' : '+'}</i></button><p className="accordion-summary">{candle.summary}</p>{active && <div className="accordion-detail" aria-live="polite"><h4>为什么判断这根{candle.close >= candle.open ? '走高' : '走低'}</h4><div className="candle-driver-grid">{candle.drivers.map((driver) => <article className={driver.impact} key={driver.label}><span>{driver.impact === 'up' ? '↑' : driver.impact === 'down' ? '↓' : '—'} {driver.label}</span><p>{driver.detail}</p></article>)}</div><div className="candle-conditions"><div><span>确认条件</span><b>{candle.confirm}</b></div><div><span>判断失效</span><b>{candle.invalidation}</b></div></div></div>}</article>; })}</div></aside>}
        </div>
      </section>
